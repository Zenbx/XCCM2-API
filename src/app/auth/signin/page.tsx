...
function SignInContent() {
    ...
    // Parse mode and provider from URL or callbackUrl
    const urlParams = new URLSearchParams(callbackUrl.includes('?') ? callbackUrl.split('?')[1] : '');
    ...
    const handleSignIn = (provider: 'google' | 'azure-ad') => {
        setSigningIn(provider);
        signIn(provider, { callbackUrl });
    };

    const isRegister = mode === 'register';
    const title = isRegister ? "S'inscrire" : "Se connecter";
                            ...
                            <div className="flex flex-col gap-4">
                                {(!provider || provider === 'google') && (
                                    <button
                                        ...
                                        className="w-full h-12 rounded-xl bg-white border border-slate-300 text-slate-700 font-semibold hover:bg-slate-50 hover:border-slate-400 transition-all flex items-center justify-center gap-3 shadow-sm active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed disabled:active:scale-100"
                                    >
                                        {signingIn === 'google' ? (
                                            <>
                                ...
                                {(!provider || provider === 'azure-ad' || provider === 'microsoft') && (
                                    <button
                                        onClick={() => handleSignIn('azure-ad')}
                                        disabled={signingIn !== null}
                                        className="w-full h-12 rounded-xl bg-[#2f2f2f] border border-[#2f2f2f] text-white font-semibold hover:bg-black transition-all flex items-center justify-center gap-3 shadow-sm active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed disabled:active:scale-100"
                                    >
                                        {signingIn === 'azure-ad' ? (
                                            <>
                                                <svg className="w-5 h-5 animate-spin text-white/60" viewBox="0 0 24 24" fill="none">
                                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" />
                                                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                                                </svg>
                                                Redirection…
